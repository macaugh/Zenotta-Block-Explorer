        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (txTableData) {
                ...
                let row = [
                    ...
                    { value: <a href={`${store.network.name}/tx/${txTableData.hash}?bnum=${txTableData.bNum}`}>{txTableData.hash}</a>, isNumeric: false },
                    { value: getTxType(txTableData.transaction.outputs), isNumeric: false },
                    { value: <a style={{ cursor: "pointer" }} href={`${store.network.name}/block/${await getBlockHashFromNum(txTableData.bNum)}`}>{txTableData.bNum}</a>, isNumeric: false },
